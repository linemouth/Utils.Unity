...
float4 SimplexNoiseGradient3D(float3 v)
{
    const float2 C = float2(1.0 / 6.0, 1.0 / 3.0);
...
void SimplexNoise3D_float(float3 position, out float Out)
{
    Out = SimplexNoise3D(position);
}
void SimplexNoiseGradient3D_float(float3 position, out float Out)
{
